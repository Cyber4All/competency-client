    ...
    */
    public sendNotificationByError(err: HttpErrorResponse){
        ...
        if (err.status !== 429){
            ...
            if (apiError.message instanceof Array && apiError.message.length > 1) {
                message = apiError.message[0].message[0].charAt(0).toUpperCase() +
                    apiError.message[0].message[0].substr(1) + ' and ' +
                    (apiError.message.length - 1) + ' more';
            } else if (apiError.message instanceof Array) {
